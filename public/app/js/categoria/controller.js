...
export const categoriaController = {
  ...
  async list(filters = {}) {
    try {
      ...
      await Swal.fire('Error', error.message || 'Error al listar usuarios', 'error');
    }
  },
  ...
  async save() {
    try {
      ...
      // Validaciones
      if (!categoria.nombre) {
        await Swal.fire('Error', 'Todos los campos son obligatorios', 'warning');
        return;
      }

      await categoriaService.save(categoria);
      await Swal.fire('Éxito', 'Categoría guardada correctamente', 'success');
      window.location.href = 'categoria/index';
    } catch (error) {
      console.error('Error al guardar categoria', error);
      await Swal.fire('Error', error.message || 'Error al guardar categoria', 'error');
    }
  },

  // Eliminar categoria (ya implementado con Swal)
  async delete(id) {
    try {
      ...
      if (result.isConfirmed) {
        ...
        await Swal.fire('Eliminado', 'Categoría eliminada con éxito', 'success');
        window.location.href = 'categoria/index';
      }
    } catch (error) {
      console.error('Error al eliminar usuario:', error);
      await Swal.fire('Error', error.message || 'Error al eliminar usuario', 'error');
    }
  },
  ...
  update: async (elements) => {
    try {
      ...
      if (!id || isNaN(id)) {
        await Swal.fire('Error', 'ID de categoría inválido', 'warning');
        return;
      }
      ...
      // Validación
      if (!cat.nombre) {
        await Swal.fire('Error', 'El nombre de la categoría es obligatorio', 'warning');
        return;
      }

      const response = await categoriaService.update(cat); // Cambia aquí también al servicio correcto
      await Swal.fire('Éxito', response.message || 'Categoría actualizada correctamente', 'success');
      sessionStorage.removeItem('originalUserData');
      window.location.href = `categoria/index`;
    } catch (error) {
      console.error('Error al actualizar la categoría:', error);
      await Swal.fire('Error', error.message || 'Error inesperado al actualizar la categoría', 'error');
    }
  },
  ...
  async exportListPDF(filters) {
    try {
      ...
      await Swal.fire('Error', error.message || 'Error al exportar a PDF', 'error');
    }
  },
